...
type IsNamedParameter<Parameter> = Parameter extends `+${infer ParamName}`
  ? ParamName extends "" ? never
  : ParamName
  : never | Parameter extends `#${infer ParamName}`
    ? ParamName extends "" ? never
    : Parameter
  : never;
...
type FilteredTopicSplit<Topic> = Topic extends `${infer PartA}/${infer PartB}`
  ? IsNamedParameter<PartA> | FilteredTopicSplit<PartB>
  : IsNamedParameter<Topic>;
...
export type MqttParameters<Topic> = {
  [key in FilteredTopicSplit<Topic> as StripParameterHash<key>]: ParameterValue<
    key
  >;
};
//#endregion
...
type JoinPath<Parts extends string[]> = Parts extends [infer v, ...infer rest]
  ? rest extends [] ? v
  : v extends string ? rest extends string[] ? `${v}/${JoinPath<rest>}`
    : ""
  : ""
  : "";
...
> = Section extends `+${infer PName}`
  ? PName extends keyof Parameters
    ? Parameters[PName] extends string ? Parameters[PName]
    : unknown
  : unknown
  : Section extends `#${infer PName}`
    ? PName extends keyof Parameters
      ? Parameters[PName] extends string[] ? JoinPath<Parameters[PName]>
      : unknown
    : unknown
  : Section;
